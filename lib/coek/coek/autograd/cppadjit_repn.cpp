...
CppADJIT_Repn::CppADJIT_Repn(Model& model) : NLPModelRepn(model)
{
    nx = 0;
    nf = 0;
    nc = 0;
...
size_t CppADJIT_Repn::num_variables() const { return nx; }

size_t CppADJIT_Repn::num_objectives() const { return nf; }

size_t CppADJIT_Repn::num_constraints() const { return nc; }

size_t CppADJIT_Repn::num_nonzeros_Jacobian() const { return jac_row.size(); }

size_t CppADJIT_Repn::num_nonzeros_Hessian_Lagrangian() const { return hes_row.size(); }

void CppADJIT_Repn::set_variables(std::vector<double>& x)
{
...
void CppADJIT_Repn::set_variables(const double* x, size_t n)
{
    assert(n == currx.size());
...
void CppADJIT_Repn::get_J_nonzeros(std::vector<size_t>& jrow, std::vector<size_t>& jcol)
{
...
void CppADJIT_Repn::get_H_nonzeros(std::vector<size_t>& hrow, std::vector<size_t>& hcol)
{
...
void CppADJIT_Repn::print_equations(std::ostream& ostr) const { NLPModelRepn::print_equations(ostr); }

void CppADJIT_Repn::print_values(std::ostream& ostr) const { NLPModelRepn::print_values(ostr); }

double CppADJIT_Repn::compute_f(size_t i)
{
    assert(i < nf);
    if (invalid_fc) {
...
void CppADJIT_Repn::compute_df(double& f, std::vector<double>& df, size_t i)
{
    assert(df.size() == nx);
...
void CppADJIT_Repn::compute_c(std::vector<double>& c)
{
    assert(c.size() == nc);
    if (invalid_fc) {
        fc_cache = ADfc.Forward(0, currx);
...
void CppADJIT_Repn::compute_dc(std::vector<double>& dc, size_t i)
{
    assert(i < fcw.size());
    assert(dc.size() == nx);
...
void CppADJIT_Repn::compute_H(std::vector<double>& w, std::vector<double>& H)
{
    if (sparse_JH) {
        //
        // Sparse Hessian
...
void CppADJIT_Repn::compute_J(std::vector<double>& J)
{
    if (sparse_JH) {
        //
...
void CppADJIT_Repn::initialize(bool _sparse_JH)
{
